...
using News_Stand.my_weather_web_service;
...
namespace News_Stand
{
    ...
    public sealed partial class weather : Page
    {
        ...
        GlobalWeatherSoapClient proxy = new GlobalWeatherSoapClient();
        public weather()
        {
        ...
        //when select combobox,user can see cities of entered countries & 
        //on click on any one city ,user can see temprature of that city
        private async void when_select(object sender, SelectionChangedEventArgs e)
        {
            try
            {
                string city = com_cities.SelectedItem.ToString().Trim();
                string weather = await proxy.GetWeatherAsync(city, country.Text);
                XmlDocument xml = new XmlDocument();
                xml.LoadXml(weather);
                XmlNodeList elemlist = xml.GetElementsByTagName("Temperature");
                temp.Text = elemlist[0].InnerText;
            }catch(Exception)
            {
                MessageDialog m = new MessageDialog("Temperature Not Found,Search Else.");
                 m.ShowAsync();
            }
        }
